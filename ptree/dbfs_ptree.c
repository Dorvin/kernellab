                                ...
                                loff_t *position)
{
        ...
        while(1){
                // Temporary store current result to result_temp to use in sprintf...
                strcpy(result_temp, result);
                // Add parent process to the top of output string
                // Make Output Format string: process_command (process_id)
                sprintf(result, "%s (%d)\n%s", curr->comm, curr->pid, result_temp);
                // If pid of curr is 1, stop
                if(curr->pid == 1){
                        break;
                }
                // going to the root of process tree
